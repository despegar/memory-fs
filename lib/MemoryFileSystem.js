...
class MemoryFileSystem {
	...
	renameSync(_oldFilePath, _newFilePath) {
		const _oldPathname = path.dirname(_oldFilePath);
		const _newPathname = path.dirname(_newFilePath);
		const _oldFileBasename = path.basename(_oldFilePath);
		const _newFileBasename = path.basename(_newFilePath);

		const oldPathname = pathToArray(_oldPathname);
		const newPathname = pathToArray(_newPathname);

		let currentOldPath = path.sep;
		let currentOld = this.data;
		oldPathname.forEach((oldPath) => {
			currentOldPath = path.join(currentOldPath, oldPath);
			currentOld = currentOld[oldPath];
			if (!isDir(currentOld)) {
				throw new MemoryFileSystemError(errors.code.ENOENT, currentOldPath, "readdir");
			}
		});

		if (!isFile(currentOld[_oldFileBasename])) {
			throw new MemoryFileSystemError(errors.code.ENOENT, path.join(currentOldPath, _oldFileBasename), "readFile");
		}

		let currentNewPath = path.sep;
		let currentNew = this.data;
		newPathname.forEach((newPath) => {
			currentNewPath = path.join(currentNewPath, newPath);
			currentNew = currentNew[newPath];
			if (!isDir(currentNew)) {
				throw new MemoryFileSystemError(errors.code.ENOENT, currentNewPath, "readdir");
			}
		});

		currentNew[_newFileBasename] = currentOld[_oldFileBasename];
		delete currentOld[_oldFileBasename];
	}
	...
	// stream methods
	createReadStream(path, optionsOrEncoding = {}) {
		...
		let readStreamOptions = null;
		if (typeof optionsOrEncoding === 'string') {
			readStreamOptions = {encoding: optionsOrEncoding};
		} else if (typeof optionsOrEncoding === 'object') {
			readStreamOptions = optionsOrEncoding;
		}
		readStreamOptions.start = readStreamOptions.start || 0;
		readStreamOptions.end = readStreamOptions.end || data.length;
		stream._read = function() {
			if (done) {
				return;
			}
			done = true;
			this.push(data.slice(readStreamOptions.start, readStreamOptions.end));
			this.push(null);
		};
		return stream;
	}
